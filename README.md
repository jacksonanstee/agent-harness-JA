...
> A local-first agent harness on the Claude Agent SDK (model routing, security guardrails, adversarial evals) with every non-trivial decision recorded: 29 ADRs, a threat model mapped to the OWASP Agentic Top 10, and a red-team gate on every PR.
...
3. **[docs/decisions/](./docs/decisions/)**: Twenty-nine ADRs (0001–0029) covering harness positioning, licence, SDK target, telemetry storage, injection scanning, secret redaction, permissions and sandboxing, the deliberately-deferred LLM judge, the golden runner, the red-team corpus, the fail-on-any-drift regression gate, the adversarial verifier, the init scaffolder, the npm publish path, the locked public API surface, the router's model tiers, refusal handling, block-on-flag enforcement for the skill channel, cleartext paths in retained sinks, the nonce-authenticated skill-section delimiter, and why the documentation gate checks structure rather than claims.
...
| ADRs | 0001–0029 |
| Tests | 1157 at the 2026-08-04 snapshot ([live status: CI](https://github.com/jacksonanstee/agent-harness-JA/actions/workflows/ci.yml)) |
