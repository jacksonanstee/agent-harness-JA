...
- **`pathDigest`** appears only on truncated rows, and is how you tell two of them apart. Keeping the tail discards everything before it, so two skills whose paths differ only in an early directory store an identical `path`. The digest covers the *full* path, so the rows stay distinguishable. Older rows predate the field and simply omit it.
