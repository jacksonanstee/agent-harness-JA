...
- **`pathDigest`** appears only on truncated rows, and is how you tell two of them apart. Keeping the tail discards everything before it, so two skills whose paths differ only in an early directory store an identical `path`. The digest covers the full *escaped* path, the same form described above and not the raw one, so the rows stay distinguishable. That distinction matters if you ever try to match a digest against a candidate file: hashing the real path will not reproduce it whenever the path contained a character that escaping rewrites, or a literal backslash, which is doubled. Older rows predate the field and simply omit it.
