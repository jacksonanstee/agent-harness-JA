...
> A local-first agent harness on the Claude Agent SDK (model routing, security guardrails, adversarial evals) with every non-trivial decision recorded: 36 ADRs, a threat model mapped to the OWASP Agentic Top 10, and a red-team gate on every PR.
...
| **Security** | Prompt-injection scanner on tool results (a flagged result is annotated for the model with a note, not withheld), plus an LLM judge to the ADR-0016 contract with a public hardened builder, measured and not yet wired into the session (ADR-0036) and on skill content (enforced when a scanner is wired, as the CLI does: a flagged skill is kept out of the system prompt), secret redaction (secrets redacted from the model's copy on a successful call, injection flagged with a note; failed-call output and tool input observed only), permission model, sandbox boundaries |
...
# Configure (needed for run/eval and redteam --judge; the red-team gate itself is keyless)
...
| ADRs | 0001–0036 |
| Tests | 1738 at the 2026-09-11 snapshot ([live status: CI](https://github.com/jacksonanstee/agent-harness-JA/actions/workflows/ci.yml)) |
