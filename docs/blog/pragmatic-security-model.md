...
- The skills loader's differential review caught a **critical RCE**: the
  ...
  `---js` frontmatter `eval()`s on load. An earlier review pass had
  concluded the default safe-loader closed that door; the wrong conclusion
  was already written down, and only a second, hostile pass re-derived it.
...
carries an assembled literal. None of this is in the threat model (the
attacker here is your own tooling), but it is the kind of friction that
...
on the fix round, and the catches recorded in the ADRs (the
[week-2 devlog](../../process/devlog/week-2.md) narrates the S-1→S-4
sequence). If your security layer's history contains no sentence of the
form "the review caught the fix breaking X", that is rarely because it
didn't happen.
