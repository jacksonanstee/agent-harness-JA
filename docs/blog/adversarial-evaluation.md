...
challenged 0 of 2 passed tasks (agreed 2) for about $0.13 of adversary
calls. An unremarkable result, which is exactly the point: a report-only
sceptic that mostly agrees is cheap, and the one time it doesn't will be
worth every prior boring run.
