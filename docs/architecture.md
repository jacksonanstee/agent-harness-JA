...
- **Public API:** `scan(text: string): ScanResult` where `ScanResult = { verdict: 'pass' | 'block' | 'ask', rule_ids: string[], excerpts: string[], suspicious: boolean }` (`suspicious` = medium-only hits, the S-5 judge trigger). Heuristic stage shipped ([ADR-0012](./decisions/0012-injection-heuristics-implementation.md)); LLM-judge (S-5) implemented as `createJudgedScanner` with a public `buildJudge` ([ADR-0036](./decisions/0036-s5-judge-implemented-and-measured.md)), not consumed by the session (its seam is the sync `scan`; wiring is PR-B of issue #96).
...
- **Design notes:** Rules are `{ tool, match?, decision }` with trailing-`*` prefix globs only. Precedence is specificity (match > tool > wildcard) then severity (deny > ask > allow), so merged layers evaluate order-independently and a user deny survives a project allow (sticky deny). Unmatched tools get `defaultDecision` (default allow). Harden with a wildcard deny RULE plus explicit allows, not a deny default: sticky deny protects user rules, and the default is consulted only when no rule in either layer matches, so a project allow rule beats a user deny default (issue #100, by execution). `ask` fails closed without a prompter; malformed settings fail loud at startup. Shipped ([ADR-0014](./decisions/0014-declarative-permission-model.md)).
...
Every step's output is recorded in `telemetry` with a turn-scoped correlation ID, so a full trace can be reconstructed after the fact — with one standing qualification: recording is best-effort and composition-dependent. A harness embedded without a `telemetry` dependency records nothing, and a failed write is downgraded to a warning rather than aborting the run. A skill dropped at step 3 is now recorded durably as a `skill-drop` event ([ADR-0011 amendment](./decisions/0011-telemetry-store-and-migrations.md), issue #46), alongside `SessionResult.droppedSkills` and the stderr warning.
