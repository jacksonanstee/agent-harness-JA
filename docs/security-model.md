...
> prompt — see §5 Tampering, §9 ASI06), a 2026-07-28 amendment for the skill
channel becoming ENFORCED rather than observe-only (ADR-0026 — see §1, the §5
Update, R-4 in §6, and §9 ASI06), a 2026-08-25 amendment for the post-tool
hook field-name fix that made the output scan and redactor actually run (the
SDK sends `tool_response`; ADR-0032, R-4) and the correction that the SDK's
rewrite channel existed all along, and a 2026-08-27 amendment for the tool
table becoming derived from the SDK's own declarations in both directions
after five declared tools were found never to have been in it (ADR-0033, R-9).
This document says
...
1. **User config vs project config.** User-level settings are trusted;
   ...
   and fail loud on malformed input before any tool runs, and since ADR-0034
   "malformed" includes an unknown key at any level inside `permissions` or
   `sandbox` (a typo of a hardening key used to fail open with no signal) and
   a command allowlist entry the shell would rewrite before exec (a glob,
   a quote, an `=` or a NUL: the program that starts is not the one the
   entry names). The settings FILE is read through the same hostile-input
   envelope as the red-team baseline: leaf and parent symlinks refused,
   single-descriptor `O_NOFOLLOW` and `O_NONBLOCK` read, anything that is
   not a regular file refused, a 1,000,000-byte cap, and a parse error that
   names the file and nothing from inside it, on both layers, with the reach
   and the residuals stated in ADR-0034.
...
project file could flip (ADR-0015 §1). A subtler tampering shape is a settings
file that WIDENS BY TYPO: a mistyped key was silently dropped, so `matchh` on
an allow rule made it a blanket allow and `sandbox.path` turned a dimension
off; since ADR-0034 every unknown key inside the two dimensions is a startup
error, and the file itself cannot be a symlink to somewhere else.
...
- **The dual-table gap.** Permissions and sandbox each kept a private
  ...
  *both* gates. Fixed with one shared table
  ...
  same failure mode recurring, not a solved problem. *Amended 2026-08-26
  (issue #86, ADR-0033): "new" was the wrong word. The table was never
  complete at the only SDK version ever pinned: five declared path/command
  tools were absent from the day the shared table landed, under a green pin
  test, and one of them (`Workflow`) is exposed in headless runs. The table is
  now derived from the SDK's own declarations by
  `tool-targets.sdk-parity.test.ts`, in both directions, with the gate's
  reach written into R-9.*
...
| R-9 | SDK tool-surface drift: any path/command-taking SDK tool absent from `tool-targets.ts` is ungated by both gates. Not a forecast: at the only SDK version ever pinned (0.3.201) five declared tools (`Projects`, `Workflow`, `Monitor`, `Artifact`, `EnterWorktree`) were absent for seven weeks under a green pin test, and `Workflow` is proven exposed in headless runs (external review 2026-08-25, issue #86) | Medium | Since #86 the table is derived, not curated: `tool-targets.sdk-parity.test.ts` reads the installed SDK declarations and fails on a declared path/command tool missing from the table, a field the SDK renamed or removed, an entry the SDK does not declare, or a stale acknowledgement of a deliberately ungated field. Its reach stops at the declarations: a tool the SDK dispatches without a declared `*Input` interface in `sdk-tools.d.ts` (the bundled native CLI names `MultiEdit`, `NotebookRead` and `Cd` in guidance and display strings, none with a tool-registration shape at 0.3.201; the table derives from the typed surface, not the binary), a target field named outside the gate's vocabulary or nested inside an object-typed field, and a caret-range SDK bump on an operator's machine (the gate runs in CI, not at run time) stay ungated | ADR-0015 §2, ADR-0033, §5 Tampering |
...
| R-13 | The command gate's exec-wrapper blocklist (shells plus `sudo`/`timeout`/`nohup`/… ) is necessarily non-exhaustive: an argv-passthrough wrapper nobody has enumerated still runs if an operator allowlists it | Medium | A blocklist over an allowlist is defence-in-depth, not the boundary — the allowlist decides which programs run at all, and the blocklist only catches known wrappers an operator allowlisted by mistake (added 2026-07-15, audit finding V10). Since ADR-0034 the basename the list keys on is the basename that RUNS for every rewrite the review could produce: glob, zsh extendedglob, quote removal, equals expansion and NUL are refused in command entries and in argv[0], closing the `/bin/s?` and `/bin/s"h"` dodges where the shell rewrote the basename after the list had passed it. The predicate is an enumeration, so a shell rewrite nobody has enumerated is R-13's own residual shape one gate over | ADR-0015 §3; ADR-0034 decision 2 |
...
| R-20 | **The skills root itself may be a symlink a clone commits, and the `run` guard refuses it only at the leaf.** `load()` realpaths the root and contains entries below it only, so a repo committing its skills directory as a symlink to an outside directory had the target scanned into the system prompt (issue #92, confirmed by execution). Closed for the default `./skills` and every single-segment or leaf case: the `run` wiring refuses a symlink at `resolve(skillsDir)` before the SDK is reached (`src/cli.ts`), and the eval path refuses the same layout by full realpath containment against the task directory (`containSkillsDir`, ADR-0017). What survives on `run` is a RELATIVE multi-segment `--skills-dir sub/skills` whose intermediate component `sub` is a repo-committed symlink: `lstat` on the leaf follows the intermediate, so leaf refusal does not see it | Low. The default and every absolute `--skills-dir` are covered; the residual needs the operator to pass a multi-segment relative path AND the clone to have committed a symlink at an intermediate component of exactly that path; the payload is still skill-schema-valid files only | Accepted. A relative-only ancestor walk (the primitive `refuseAncestorSymlinks` exists for repo-internal relative paths) would close it, but was deferred: an absolute-path ancestor walk would false-reject the legitimate operator paths under OS-owned symlinks that motivate leaf-only here, and a relative walk's `lstat` resolves against the real cwd, which cannot be redirected under the test harness, so the closure ships without a driven test. Revisit when `run` gains a skills-dir that must be repo-relative | issue #92; ADR-0006 amendment; ADR-0017; `src/cli.ts`; `src/internal/guarded-read.ts` |
