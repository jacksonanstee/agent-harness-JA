...
token ceiling, no dollar budget and no cancellation channel, although the
pinned SDK declares `abortController`, `maxBudgetUsd` and an alpha
`taskBudget` (requirements H-6 and H-7). Beneath every turn the bundled CLI
retries a failed API request on its own, with its own retry count and
per-request timeout, and reports each retry as an `api_retry` message the
session loop does not read, so a stalled or rate-limited run is
indistinguishable from a clean one at every harness sink (H-8). The harness
installs no signal handler, so an operator's Ctrl-C ends the process without
the stop hook or the turn-cost row, and an interrupted run leaves no cost
trace. Cost hygiene on the same basis as the pack bounds above; tracked as
R-21.
