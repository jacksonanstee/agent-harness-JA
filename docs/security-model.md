...
| R-15 | A refusal retried on a **fallback model** returns a genuine answer from a model the router did not choose, and `SessionResult.modelChoice` plus the telemetry `model` field still name the routed one. The harness never sets `fallbackModel` itself, but at least three channels outside its control can: the operator's own ambient configuration (R-11), a `fallbackModel` in **managed settings**, which the SDK's settings schema describes as typically administered by enterprise administrators rather than by the operator running the harness, and a `--fallback-model` CLI flag the SDK documents as taking precedence over both. The SDK also refers to server-side gating of the fallback path ("declined or gate-failed", a "server-gated trigger"), whose conditions it does not document, so this list is **not claimed to be exhaustive** | Medium | Detected and reported rather than prevented: `refusal.fallbackModel` names the answering model, a warning fires, and `run` annotates the model claim on stdout as well as printing the refusal line to stderr. The run still exits 0, because there is a real answer and a non-zero exit would misreport the outcome (ADR-0025 decision 4). Rated Medium rather than Low because the enterprise-managed channel means an operator can inherit a fallback they never configured and cannot see, and because it composes with R-4: the answering model's retention posture may not be the one ADR-0024 decision 1 vetted for the routed model | ADR-0025 decision 4, §6 R-4/R-11; `src/session/session.ts` |
