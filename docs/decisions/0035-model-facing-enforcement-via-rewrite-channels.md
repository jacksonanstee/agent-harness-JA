...
The design rests on twelve facts verified against the tree at main `44f80cf` and, where marked, against the pinned SDK by execution. The load-bearing ones:
...
- `unobserved`: neither appears, both appear, the content cannot be walked, or no user message for that id arrived before the stream ended. The row carries a `reason` (`no-user-message`, `both-present`, `unwalkable` or `neither-token`) so one word does not fold four causes.
...
- The rewrite subsystem grows again: `session.ts` grew past 1800 lines with this change, and the pure, state-free parts (the walk, the constants, `decideRewriteOutcome`, the notice/marker helpers) extract cleanly to `src/session/rewrite.ts`. The architecture lens raised it (A-1, MEDIUM); it is filed as issue #138 rather than bundled here, because a 200-line move of just-reviewed security code deserves its own review.
