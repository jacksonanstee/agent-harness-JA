...
- The rewrite subsystem grows again: `session.ts` reached 1879 lines with this change, and the pure, state-free parts (the walk, the constants, `decideRewriteOutcome`, the notice/marker helpers) extract cleanly to `src/session/rewrite.ts`. The architecture lens raised it (A-1, MEDIUM); it is filed as issue #138 rather than bundled here, because a 200-line move of just-reviewed security code deserves its own review.
