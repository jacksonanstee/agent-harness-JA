...
3. **Any future attempt must not key on an ambient `os.homedir()`.** It must either record the home value in the row alongside the transformed text, so a reader can tell whether the transform applied and to what, or take the home directory as an explicit operator argument with no environment default. A transform that cannot report whether it fired is not a mitigation.
