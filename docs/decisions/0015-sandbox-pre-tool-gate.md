...
The gated tool surface lives in ONE shared table (`src/internal/tool-targets.ts`), consumed by both the sandbox and the permissions evaluator. The review of the first cut found each module keeping a private four-tool table while assuming the other covered the rest — `Glob`, `Grep`, `NotebookEdit` and `MultiEdit` bypassed **both** gates. The shared table covers all path/command-taking SDK tools and is pinned by a test, so extending it is a visible act. *Amended 2026-08-26 (issue #86, ADR-0033): "covers all" was false at the only SDK version the harness has run. Five declared tools (`Projects`, `Workflow`, `Monitor`, `Artifact`, `EnterWorktree`) were never in the table, and `MultiEdit`, listed here, was never declared by the SDK. The table is now derived from `sdk-tools.d.ts` by a parity test in both directions and `MultiEdit` left it; ADR-0033 records the shape decision, the absent-optional-field rule and the gate's stated reach.* Network-egress tools (`WebFetch`/`WebSearch`) are deliberately absent: they need a URL/domain dimension, not a path prefix (Revisit if).
...
What a string-level allowlist can truthfully bound is *which program starts*. When enabled: empty/whitespace commands deny; any command containing a shell metacharacter (`; | & $ ` ( ) { } < > \n \r \ !`) denies outright — once those appear we can no longer name the program, so we refuse rather than pretend to parse shell (`\` escape/continuation and `!` history expansion added in review; `*`/`~` deliberately excluded — glob/tilde expansion happens inside the allowed program's argument list and does not change which program starts). *Amended 2026-08-28 (ADR-0034 decision 2, issue #93): true of the ARGUMENTS, false of argv[0]. A glob-shaped allowlist entry (`/bin/s?`, `/bin/*`, `[s]h`) passed the parser and the blocklist as written, matched an argv[0] of the same string exactly, and the executing shell expanded it to `/bin/sh` (executed under bash and zsh). Every character the shell rewrites in a word before exec (`*`, `?`, `[`, the zsh extendedglob `^` and `#`, the quotes `"` and `'` that quote removal strips, `=` for zsh equals expansion and assignment prefixes, and NUL) is now refused in command allowlist entries at parse time and in argv[0] at the gate, through one shared predicate; the security review of ADR-0034 produced `/bin/s"h"` and `=sh` as further members of the class by execution. Arguments keep their allowance.* Otherwise argv[0] is compared exactly against entries: entries containing `/` compare post-`resolve` against path-style invocations only; bare names match bare names only (`git` never matches `/tmp/git`). Layer intersection uses this same identity grammar (review fix: blanket `resolve` had made bare `git` equal `./git`).

**Shell runners and exec wrappers are a hard blocklist, not a warning.** `sh`, `bash`, `zsh`, `dash`, `ksh`, `csh`, `fish`, `env`, and `xargs` are denied by basename — case-folded on darwin/win32 (*amended 2026-07-08, Week-2 milestone review: `SH -c` resolves the same binary as `sh -c` on APFS/NTFS, and a blocklist documented as unconditional must not be case-dodgeable; the path-shaped allowlist compare and command entry-identity moved from raw `resolve` to `canonicalizePath` in the same pass so all three share the path gate's identity grammar*) — even when an allowlist names them (review escalation: `bash -c '…'` makes argv[0] analysis meaningless by construction, and a warn-only stance was security theater for a bypass class this ADR names itself). *Amended 2026-07-15 (audit finding V10): argv-passthrough wrappers join the same blocklist — they exec their trailing argv as a subprocess exactly like `env`, so an allowlisted `sudo git` runs the real command behind an approved argv[0]. The list groups privilege (`sudo`/`doas`/`su`/`runuser`/`pkexec`), run-and-supervise (`timeout`/`nohup`/`setsid`/`time`), scheduler (`nice`/`ionice`/`taskset`/`chrt`), namespace/root (`chroot`/`unshare`/`nsenter`) and stream/lock (`stdbuf`/`flock`) wrappers; see `EXEC_WRAPPER_BINARIES` for the current set (necessarily non-exhaustive — R-13).* The CLI still surfaces the conflict as a startup warning so the settings author learns why their entry is inert. *Amended 2026-08-28 (ADR-0034 decision 2): "unconditional" held for the basename as WRITTEN, not as executed; a glob-shaped entry whose basename the shell rewrites to a listed shell dodged the list. Closed by refusing the characters the shell rewrites in entries and in argv[0], so the basename the blocklist keys on is the basename that runs, for every rewrite the review could enumerate.*

**What this cannot stop** (documented non-goals): interpreter escapes (`node -e`, `python -c`) when those binaries are allowed; argv-level execution (`find -exec`, `git -c core.fsmonitor=…`); glob expansion inside an allowed program's arguments; anything an allowed program does internally. The exec-wrapper blocklist is necessarily non-exhaustive — a wrapper nobody has enumerated still passes if it is allowlisted (residual R-13); the allowlist, not the blocklist, is the primary boundary. Real containment is an OS sandbox and out of scope for v1.
...
The read → ENOENT-is-empty → JSON-parse-fail-loud → path-prefixed-rethrow mechanics moved from `permissions/settings.ts` to the shared internal leaf (`loadJsonSettings<T>`), closing the S-3 architecture finding before a third module copies it. Policy (what keys mean) stays in each module's parser; permissions' public `loadSettingsFile` signature is unchanged and its tests pass unmodified — behavioural proof of the hoist. The CLI reads each settings file **once** and feeds the parsed doc to both key-parsers; malformed anything → exit 2 before any tool runs. *Amended 2026-08-28 (ADR-0034 decisions 3 to 5, issue #94): the read itself is now the hostile-file envelope the red-team baseline loader already had, hoisted to `src/internal/guarded-read.ts` on this third consumer: leaf and parent symlinks refused, single-descriptor `O_NOFOLLOW` read, a 1,000,000-byte cap, and a parse error that names the file and nothing from inside it. `composeSecurity` defaults to that reader; the injectable `readFile` is a test seam.*
...
- A third settings consumer appears: `loadJsonSettings` may grow layer-discovery (`user`/`project` path resolution) too. *Fired 2026-08-28 in a different shape: the third consumer was of the symlink GUARD, not of the loader, and what grew was the read envelope (ADR-0034 decision 4). Layer discovery still lives in `composeSecurity`.*
