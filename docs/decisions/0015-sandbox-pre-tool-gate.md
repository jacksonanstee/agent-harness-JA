...
**Shell runners are a hard blocklist, not a warning.** `sh`, `bash`, `zsh`, `dash`, `ksh`, `csh`, `fish`, `env`, and `xargs` are denied by basename even when an allowlist names them (review escalation: `bash -c '…'` makes argv[0] analysis meaningless by construction, and a warn-only stance was security theater for a bypass class this ADR names itself). The CLI still surfaces the conflict as a startup warning so the settings author learns why their entry is inert.
