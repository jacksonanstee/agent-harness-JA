...
1. **Contract, one line: `taskDir` is populated only when the task directory is at or under the working directory the run was invoked from. Any other invocation stores null.** The classifier is pure string logic over `rel = relative(cwd, root)`: empty maps to `'.'`; a form that is not absolute and whose first segment is not `..` is stored as-is; everything else suppresses. Suppression is the fall-through, not a positive match, so a shape the classifier does not recognise fails safe by removing information rather than disclosing it. The absolute-paths precondition on both arguments is ENFORCED, not trusted: a non-absolute argument suppresses outright. The verify round executed the mis-call that forced this — with relative arguments `relative()` re-anchors both operands to the ambient `process.cwd()`, and `portableTaskDir('.', '../'.repeat(14))` from a deep working directory POPULATED the operator's home-relative path while passing both the absolute and leading-`..` checks. A docstring-only precondition was a false safety claim; the guard makes it true by construction, in the same information-removing direction as every other refusal here.
...
3. **The signal is `taskDirForm: 'relative' | 'suppressed'`, always present.** This is ADR-0027 decision 3's "boolean paired with an identifier for the transform that ran", encoded as a two-value union because there is exactly one transform to name; the value says whether it applied ('relative') or refused ('suppressed'). It is named `taskDirForm` rather than following the repo's `*Kind` discriminants because it names which of two forms the field pair is in, not a kind of failure or event. Nothing forbidden is recorded: no home value, no cwd. It is a discriminated union with `taskDir` — `{ taskDir: string; taskDirForm: 'relative' } | { taskDir: null; taskDirForm: 'suppressed' }` — so the invalid pairings are unrepresentable rather than merely documented. Two independently-settable fields whose relationship lived in prose already cost this codebase a review round once (`pathTruncated`/`pathDigest`, ADR-0011), and a golden scorecard has no read-time validator to retrofit a coherence check into, so the type level is the only defence available.
...
- Fall-through flipped to populate (one branch short of a full always-populate stub: the empty-string branch keeps its `'.'` mapping, which the fourth mutant covers — the first, second and fourth mutants together cover all three classifier return sites; the precondition guard's return is bound by its own mutant below): exactly the 5 suppression tests red — sibling, parent, HOME-independence, the escape shape, and the end-to-end outside-directory test.
...
- Precondition guard removed: exactly the non-absolute-argument test red, and the verify round's counterexample re-executed against the guarded build returns suppressed.

**Limits.** The Windows cross-drive branch (`isAbsolute(rel)` → suppress) is reasoned, not executed: there is no Windows CI, and on POSIX `relative()` never returns an absolute path, so no test on this platform can bind that condition. It fails in the safe direction. Decision 6's warning line is smoke-verified only: no automated test drives `runEval`'s report path (the CLI is the composition root and hardwires the real SDK; the suite deliberately mocks no modules), so deleting the warning branch leaves the suite green — declared here rather than left to read as bound. All measurements are darwin, ASCII paths.
