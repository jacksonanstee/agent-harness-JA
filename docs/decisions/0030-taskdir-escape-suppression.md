...
- Fall-through flipped to populate (one branch short of a full always-populate stub: the empty-string branch keeps its `'.'` mapping, which the fourth mutant covers — the first, second and fourth mutants together cover all three return sites): exactly the 5 suppression tests red — sibling, parent, HOME-independence, the escape shape, and the end-to-end outside-directory test.
