...
Two layers: user `~/.harness/settings.json`, then project `<cwd>/.harness/settings.json`. Each layer resolves its own winner under §2; the final decision is the **most severe** of the layer winners. A project layer can therefore tighten the user's policy but can never loosen it — *regardless of specificity* *(Corrected 2026-09-04, issue #100: true of a user deny rule; a user deny default is not sticky, see the §3 correction)*. This matters because project settings are attacker-influenced input (a cloned repo ships its own `.harness/settings.json`): with naive combined-list evaluation, `{tool:'Bash', match:'*', decision:'allow'}` in a project file would out-specific a user's blanket Bash deny and silently defeat it. Found by security review of the first cut; the per-layer/max-severity combination closes it and is regression-tested. `defaultDecision` is scalar and less dangerous (it only applies when *no* rule matches): project overrides user. File shape is `{ "permissions": { "defaultDecision"?, "rules": [] } }`, leaving the file open for future settings keys (unknown siblings ignored). Rule lists are capped at 1000 entries per file. *Amended 2026-08-28 (ADR-0034 decision 1, issue #85): "unknown siblings ignored" means siblings of `permissions` at the ROOT of the file, and only those. Inside `permissions`, at every object level, an unknown key is an error: the parser had been dropping them, so `defaultDecison` left the default at allow and a mistyped `match` widened a scoped allow rule to a blanket one, each with no signal, and a test pinned the ignore.*

### 6. Malformed settings fail loud at startup

A missing settings file is an empty layer. A file that exists but is invalid (bad JSON, bad schema) throws `PermissionSettingsError` before any tool runs and the CLI exits 2. Silently skipping a broken security config would fail open; crashing is the conservative behaviour. Validation is hand-rolled (no schema dependency), matching S-1/S-2 style; bad entries under `permissions` are errors, never skipped. *Amended 2026-08-28: including unknown keys at every level inside `permissions` (ADR-0034 decision 1), and the file read itself is now guarded (symlink refusal, byte cap, a parse error that names the file and nothing from inside it; ADR-0034 decision 3).*
...
- Default-allow means the module protects nothing until a settings file exists. Accepted: documented hardening is one line, and breaking every existing flow by default is worse (§3). *(Corrected 2026-09-04, issue #100: the one-line recipe does not survive a project allow rule; see the §3 correction.)*
