...
**Amendment (2026-07-14, Week 4 dogfooding): the body now actually reaches the agent.** This ADR's contract line — "Markdown body. This is what the agent reads when the skill is loaded." — was not true of the implementation until Week 4: `buildSystemPrompt` injected only `name: description` lines, so a skill's knowledge never entered the model's context. The gap was invisible to every unit test (they asserted on names and descriptions) and was caught within an hour of building the first real example agent (`examples/repo-qa`), whose golden task failed because the model could not see the skill body it was told to answer from. Fixed in `src/session/session.ts`: the full body is injected per skill under a `## Skill: <name>` header, stripped through the same `cleanSkillText` charset contract as the header fields (**both clauses superseded 2026-08-02 by [ADR-0028](./0028-skill-section-nonce-delimiter.md): the header now carries a per-run nonce, `## Skill [<nonce>]: <name>`, and the body goes through `cleanSkillBody`, a deliberately DIFFERENT charset contract that preserves newlines and tabs**), and the injection scanner now runs on the RAW body (separately labelled from the description scan) before stripping — observe-only, same R-4 posture as tool output — **superseded 2026-07-28 by [ADR-0026](./0026-skill-channel-block-on-flag.md), which makes a high-confidence block drop the whole skill**. Note the surface this widens: a hostile skill body is now direct system-prompt input, which is precisely why body scanning landed in the same change and why `docs/security-model.md`'s untrusted-clone attacker model applies to skill packs (the R-4 widening is risk-accepted there, §5/§6, same date).

Two boundaries from the same change: (1) *prompt assembly is owned by the session adapter* — `buildSystemPrompt` lives in `src/session/session.ts` under [ADR-0010](./0010-sdk-session-adapter.md); this ADR owns the format contract ("the body is what the agent reads"), ADR-0010 owns how it reaches the SDK. (2) *aggregate size budget* — the loader's 1 MB cap is per file; `buildSystemPrompt` additionally enforces an aggregate cap across all injected skills (`MAX_SKILL_PROMPT_CHARS`, whole-skill drop + warning — never a mid-body truncation), because `body` has no schema `maxLength` and a schema-valid pack could otherwise inject unbounded content into every turn. Revisit if skills ever load off the startup path (hot reload, long-running server) or if legitimate packs hit the budget: the cap should become configurable before it becomes a lie.
