...
| H-7 | COULD | A running session can be bounded by a token ceiling and a wall-clock ceiling, and cancelled from outside, with the partial result, the stop reason and the cost still recorded. *(Added 2026-09-07 from the 2026-08-25 external review, issue #101. Today the only bound the harness passes to the SDK is `maxTurns`. The pinned SDK declares `abortController` and `maxBudgetUsd` on its `query()` options, plus an alpha `taskBudget` that paces the model rather than stopping it; the harness's `QueryFn` seam carries none of them, so a run cannot be cancelled through the harness and its only whole-run cap is a turn count (the SDK's own per-request timeout and retries bound each request, not the run). The dollar budget stays H-6. ADR-0017 and ADR-0020 record the wall-clock gap as a limitation.)* | Deferred to v1.x. |
| H-8 | COULD | The harness states what happens when the SDK call hangs or fails transiently: a per-run timeout, whether it retries and how many times, and a telemetry record of every retry. *(Added 2026-09-07, issue #101. Today the SDK retries a failed API request on its own and declares an `api_retry` system message for each retry; the session loop has no branch for that message, and the SDK result's `duration_ms` and `duration_api_ms` are declared but not read, so a retried run is indistinguishable from a clean one at every harness sink. Timeout and retry stances exist in the record for the adversary call (ADR-0020), the injection judge (ADR-0016), the refusal fallback (ADR-0025) and hook handlers (ADR-0008, ADR-0010); none of them states what the primary session does when its own SDK call hangs or fails.)* | Deferred to v1.x. |

## Security layer

| ID | Priority | Requirement | Verification |
|---|---|---|---|
| S-1 | MUST | All tool-call return values pass through a prompt-injection scanner before being surfaced to the agent. *(Note 2026-09-08, issue #84 (ADR-0035): a block or ask verdict now annotates the model's copy with a plain note through the SDK `additionalContext` channel (D2); the flagged output is still shown, and withholding stays the judge's call, issue #96. A FAILED tool call fires PostToolUseFailure, which was unhooked until D8 added the scan there, so S-1 had been false for failed calls since the post-tool hook was wired.)* | Red-team corpus pass rate ≥90% on the injection subset. |
...
| E-3 | MUST | Red-team results are committed as a canonical-JSON baseline (`eval/redteam/baseline.json`); regression detection diffs every run against it and CI fails on any drift. *(Amended from "persisted in SQLite with a stable schema" — deviation recorded in ADR-0019: the E-1/E-2 canonical-JSON scorecard superseded the database.)* | Baseline schema validated on load (ajv allowlist); regression test detects a deliberately-broken baseline (corrupted-fixture tests in `baseline.test.ts` + CLI compare-path negative test in `redteam-command.test.ts`). |
| E-4 | SHOULD | A two-pass adversarial verification module is available as a runtime guardrail OR offline eval, with the second pass model pluggable. *(Resolved to offline eval, report-only, enum-confined findings — ADR-0020.)* | Live operator acceptance run (Claude as both primary and adversary, result in PR/devlog); CI enforces the fake-adversary differential invariance test. |
