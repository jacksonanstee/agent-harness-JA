...
| H-8 | COULD | The harness states what happens when the SDK call hangs or fails transiently: a per-run timeout, whether it retries and how many times, and a telemetry record of every retry. *(Added 2026-09-07, issue #101. Today the SDK retries a failed API request on its own and declares an `api_retry` system message for each retry; the session loop has no branch for that message, and the SDK result's `duration_ms` and `duration_api_ms`, which a retry's delay would show, are not read, so a retried run is indistinguishable from a clean one at every harness sink. Timeout and retry stances exist in the record for the adversary call (ADR-0020), the injection judge (ADR-0016), the refusal fallback (ADR-0025) and hook handlers (ADR-0008, ADR-0010); none of them states what the primary session does when its own SDK call hangs or fails.)* | Deferred to v1.x. |
