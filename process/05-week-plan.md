...
- [x] **Regression detection** (E-3) — canonical-JSON diff of every run against the committed baseline (`eval/redteam/baseline.json`); CI fails on ANY drift, classified per row. *(2026-07-12, ADR-0019; the original "SQL diff" phrasing was superseded by the E-1/E-2 canonical-JSON scorecard — deviation recorded in ADR-0019.)*
- [x] **Adversarial verification** (E-4) — offline, report-only, enum-confined second-pass challenge over oracle-pass golden rows (`eval --challenge`); pluggable adversary satisfied by an injected `AdversaryFn`, router-pinned to a second Claude model (no cross-provider in v1). *(2026-07-12, ADR-0020; C fallback trigger EOD 2026-07-16 did not fire — E-4 shipped ahead of it.)*
- [x] `docs/eval-methodology.md` — how scoring works, what counts as a regression, how to author new cases. *(2026-07-13; also carries the ADR-0020 §3 challenge-rate definition + synthetic rendered example + exit-invariance-by-composition note, per the E-4 round-2 finding R2-10 and the milestone differential review's deferred item.)*

**Checkpoint:** `npx agent-harness-ja eval` produces a Markdown scorecard. The every-PR CI gate on the red-team arm is `falseBlockCount === 0` (no benign input is ever blocked) **plus no drift against the committed baseline** (`eval/redteam/baseline.json`; fail-on-any-drift, classified per row — ADR-0019) — not a pass-rate threshold; gating on a detection percentage would strand honest new cases the scanner misses and disarm the same number ADR-0016 §6's S-5 trigger needs to measure (ADR-0018). Detection rate with default security on (≥90% measured at E-2 design time) and the security-off baseline (a null scanner detects 0/40 by construction — a guaranteed-zero control, not a measured differential) are **reported** scorecard metrics that feed the S-5 decision, not merge blockers. CI runs eval on every PR (the deterministic red-team arm only — golden eval needs a live key and executes repo oracle code, so it never runs in per-PR CI; ADR-0016 §7, ADR-0017, ADR-0018).
